...
class _AnalyticsTabState extends State<AnalyticsTab> {
        ...
        children: [
          Padding(
            ...
            child: Column(
              ...
              children: [
                _buildPeriodRow(),
                if (_selectedFilter == 3 && _customFrom != null) ...[const SizedBox(height: 12), _buildCustomBadge()],
                const SizedBox(height: 20),
                _buildDrilldownTabBar(),
                const SizedBox(height: 20),
                if (_isLoading)
                  const Center(child: Padding(padding: EdgeInsets.all(32), child: CircularProgressIndicator(color: Colors.indigo)))
                else if (_drilldownTab == 0)
                  _buildSummaryView()
                else
                  _buildCallsView(),
                const SizedBox(height: 28),
                if (!_isLoading && _filteredLogs.isNotEmpty) _buildAnalyticsSection(),
  ...
  Widget _buildPeriodRow() {
    final labels = ['Today', 'Yesterday', 'Week', 'Custom'];
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: List.generate(labels.length, (i) {
        final active = _selectedFilter == i;
        return GestureDetector(
          onTap: () => i == 3 ? _pickCustomRange() : setState(() { _selectedFilter = i; _customFrom = null; _customTo = null; }),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
            decoration: BoxDecoration(
              color: active ? Colors.indigo : Colors.white,
              borderRadius: BorderRadius.circular(100),
              border: Border.all(color: active ? Colors.indigo : Colors.grey.shade300),
            ),
  ...
  Widget _buildCustomBadge() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.date_range, size: 15, color: Colors.indigo.shade400),
          const SizedBox(width: 8),
          Text('$from — $to', style: TextStyle(fontSize: 13, color: Colors.indigo.shade700, fontWeight: FontWeight.w500)),
          const Spacer(),
          GestureDetector(onTap: _pickCustomRange, child: Text('Change', style: TextStyle(fontSize: 13, color: Colors.indigo.shade600, fontWeight: FontWeight.bold))),
        ],
      ),
    );
  }
        ...
        child: Row(children: [
          ...
          if (badge != null) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: active ? Colors.indigo.shade100 : Colors.grey.shade200,
                borderRadius: BorderRadius.circular(10),
              ),
              child: Text('$badge', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: active ? Colors.indigo.shade700 : Colors.grey.shade600)),
            ),
          ],
        ]),
      ),
  ...
  Widget _statsCard(Map<String, int> s) {
    return Container(
      ...
      child: Column(children: [
        _sRow('Total Calls',       '${s['total']}',    bold: true),
        _sRow('Call Duration',     _fmtDur(s['totalDur']), bold: true),
        const Divider(height: 1, color: Color(0xFFE5E7EB)),
        _sRow('Incoming',          '${s['incoming']}', color: Colors.blue.shade600),
        _sRow('Incoming Duration', _fmtDur(s['inDur']), color: Colors.blue.shade300),
        const Divider(height: 1, color: Color(0xFFE5E7EB)),
        _sRow('Outgoing',          '${s['outgoing']}', color: Colors.green.shade600),
        _sRow('Outgoing Duration', _fmtDur(s['outDur']), color: Colors.green.shade300),
        const Divider(height: 1, color: Color(0xFFE5E7EB)),
        _sRow('Missed',            '${s['missed']}',   color: Colors.red.shade500),
        _sRow('Rejected',          '${s['rejected']}', color: Colors.orange.shade500),
        const Divider(height: 1, color: Color(0xFFE5E7EB)),
  ...
  Widget _metaCard() {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        _mRow('Device', _deviceModel, valColor: Colors.indigo.shade600),
      ]),
    );
  }
  ...
  Widget _callItem(CallLogEntry e) {
    ...
    Color color; String typeStr;
    switch (t) {
      case CallType.incoming:  color = Colors.blue.shade500;   typeStr = 'incoming';  break;
      case CallType.outgoing:  color = Colors.green.shade500;  typeStr = 'outgoing';  break;
      case CallType.missed:    color = Colors.red.shade400;    typeStr = 'missed';    break;
      case CallType.rejected:  color = Colors.orange.shade500; typeStr = 'rejected';  break;
      default:                 color = Colors.grey;            typeStr = 'unknown';
    }
    ...
    return Padding(
      ...
      child: Row(children: [
        // Badge
        Container(
          width: 68, padding: const EdgeInsets.symmetric(vertical: 4),
          decoration: BoxDecoration(color: color.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(6)),
          alignment: Alignment.center,
          child: Text(typeStr, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: color, letterSpacing: 0.4)),
        ),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Color(0xFF111827))),
          if (hasSub) Text(e.number ?? '', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(_fmtTime(e.timestamp), style: TextStyle(fontSize: 12, color: Colors.grey.shade500, fontWeight: FontWeight.w500)),
          if ((e.duration ?? 0) > 0)
            Text(_fmtDur(e.duration), style: TextStyle(fontSize: 11, color: Colors.grey.shade400)),
        ]),
      ]),
    );
  }
  ...
  Widget _chartTogBtn(String label, int idx, IconData icon) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(icon, size: 13, color: active ? Colors.indigo : Colors.grey.shade500),
          const SizedBox(width: 4),
          Text(label, style: TextStyle(fontSize: 11, fontWeight: active ? FontWeight.w700 : FontWeight.w500, color: active ? Colors.indigo : Colors.grey.shade600)),
        ]),
      ),
    );
  }
  ...
  Widget _buildPieChart(Map<String, int> s) {
    final data = [
      ...
      (s['missed']!,   const Color(0xFFF87171)),  // red
  ...
  Widget _buildBarChart(Map<String, int> s) {
    ...
    final colors = [const Color(0xFF3B82F6), const Color(0xFF22C55E), const Color(0xFFF87171), const Color(0xFFF59E0B)];
      ...
      BarChartData(
        ...
        barGroups: List.generate(vals.length, (i) => BarChartGroupData(x: i, barRods: [
          BarChartRodData(toY: vals[i].toDouble(), color: colors[i], width: 32, borderRadius: BorderRadius.circular(6),
            backDrawRodData: BackgroundBarChartRodData(show: true, toY: maxY == 0 ? 1 : maxY * 1.3, color: colors[i].withValues(alpha: 0.08))),
        ])),
      ),
    ...
    return LineChart(
      LineChartData(
        ...
        lineTouchData: LineTouchData(enabled: true, touchTooltipData: LineTouchTooltipData(
          getTooltipItems: (pts) => pts.map((p) => LineTooltipItem('${p.y.toInt()} calls', const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12))).toList(),
        )),
        gridData: FlGridData(show: true, drawVerticalLine: false,
          getDrawingHorizontalLine: (_) => FlLine(color: const Color(0xFFE5E7EB), strokeWidth: 1)),
        borderData: FlBorderData(show: false),
        titlesData: FlTitlesData(
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, interval: (keys.length / 4).ceilToDouble(),
            getTitlesWidget: (v, _) {
              final i = v.toInt();
              return i < keys.length ? Padding(padding: const EdgeInsets.only(top: 6), child: Text(keys[i], style: TextStyle(fontSize: 9, color: Colors.grey.shade500))) : const SizedBox();
            })),
        ...
        lineBarsData: [
          LineChartBarData(
            ...
            belowBarData: BarAreaData(show: true, gradient: LinearGradient(
              colors: [Colors.indigo.withValues(alpha: 0.3), Colors.indigo.withValues(alpha: 0.0)],
  ...
  Widget _buildLegend(Map<String, int> s) {
    final items = [
      ...
      ('Missed',   const Color(0xFFF87171), s['missed']!),
